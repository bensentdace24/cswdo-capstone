                ...
                <div class="row align-items-center">
                    <div class="col-sm-6">
                        ...
                        <h1 class="text-3xl font-semibold text-gray-800"
                            style="font-size: 2rem; font-weight: 500; color: #333; display: inline-block; margin-right: 10px;">
                            {{ $header_title }}
                        </h1>
                        <span style="font-size: 1rem; color: #6c757d; font-weight: 400;">({{ $dependents->total() }}
                            Records)</span>
                    </div>

                    <div class="col-sm-6 text-right d-flex justify-content-end align-items-center">
                        @isset($client)
                            <a href="{{ url('staff/client_dependents/add/' . $client->id) }}" class="btn btn-primary"
                        ...
                        <a href="{{ url('staff/client/list') }}" class="btn btn-secondary"
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card shadow-sm" style="border-radius: 12px; border: none; overflow: hidden;">
                            <div class="card-header bg-white border-b" style="padding: 15px 20px;">
                                <h3 class="card-title text-xl font-medium text-gray-800"
                                    style="font-size: 1.5rem; font-weight: 40000;">List of Patient Dependents</h3>
                            </div>
                            ...
                            <div class="card-body p-0" style="overflow-x: auto;">
                                <table class="table table-hover table-striped">
                                    <thead>
                                        <tr style="background-color: #f8f9fa;">
                                            <th
                                                style="border-bottom: 2px solid #e9ecef; color: #495057; padding: 15px 20px;">
                                                #</th>
                                            <th
                                                style="border-bottom: 2px solid #e9ecef; color: #495057; padding: 15px 20px;">
                                                Dependent Name</th>
                                        ...
                                        @forelse($dependents as $dependent)
                                            <tr class="hover:bg-gray-50 transition duration-100">
                                                ...
                                                <td style="padding: 12px 20px; font-weight: 600;">
                                                    {{ $dependent->dependent_name }}</td>
                                                {{-- Set remaining data cells to normal or lighter weight --}}
                                                <td style="padding: 12px 20px; font-weight: 500; color: #333;">
                                                    {{ $dependent->age }}</td>
                                                <td style="padding: 12px 20px; font-weight: 400; color: #555;">
                                                    {{ $dependent->status }}</td>
                                                <td style="padding: 12px 20px; font-weight: 400; color: #555;">
                                                    {{ $dependent->relationship }}</td>
                                                <td style="padding: 12px 20px; font-weight: 400; color: #555;">
                                                    {{ $dependent->occupation }}</td>
                                                <td style="padding: 12px 20px; font-weight: 500; color: #333;">
                                                    {{ date('d-m-Y', strtotime($dependent->birthday)) }}</td>
                                                <td style="padding: 12px 20px; font-weight: 400; color: #555;">
                                                    {{ date('d-m-Y H:i A', strtotime($dependent->created_at)) }}</td>
                                                <td style="padding: 12px 20px;">
                                                    <div class="d-flex">
                                                        {{-- Action Buttons (Styles remain the same) --}}
                                                        <a href="{{ url('staff/client_dependents/edit/' . $dependent->id) }}"
                                                        ...
                                                        <a onclick="return confirm('Are you sure you want to delete this patient dependent?')"
                                                            href="{{ url('staff/client_dependents/delete/' . $dependent->id) }}"
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="9" class="text-center py-4 text-gray-500"
                                                    style="font-style: italic;">No dependents found.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                                ...
                                <div class="card-footer clearfix bg-white"
                                    style="padding: 15px 20px; border-radius: 0 0 12px 12px; border-top: 1px solid #eee;">
